...
// Il quadro completo di un giocatore: catalogo, piu' la crescita maturata con
// l'overall, piu' l'eventuale specializzazione che vince su tutto.
//
// Serve ai profili tattici, che devono leggere il giocatore di ADESSO e non
// quello dell'importazione — e che devono poter rispondere "non lo so" invece
// di far saltare la giornata, quindi qui non si pretende che un attributo ci
// sia. La specializzazione non viene fatta crescere: e' gia' un valore
// assoluto calcolato partendo dagli attributi cresciuti.
function attributiEffettivi(
...
function adaptPlayer(instance: Instance, player: DbPlayer, crescita: Crescita): EnginePlayer {
  ...
  const quadroCompleto = attributiEffettivi(
    player.attributi, instance.attributi_override, posizioni, deltaOverall, crescita)
