-- ============================================================
--  UN RINNOVO PER STAGIONE (design §10.4 bis)
--
--  Richiesta dell'utente: chi ha appena rinnovato non può ritrattare subito.
--  Se ne riparla dalla stagione successiva.
--
--  Senza questo vincolo il rinnovo è ripetibile all'infinito nella stessa
--  stagione: firmato un contratto, si riapre la scheda e si ricontratta —
--  e siccome firmare azzera i tentativi, si azzererebbe anche il costo di
--  quella scelta. Il limite dei tre tentativi resterebbe aggirabile.
--
--  Nuova colonna rinnovo_stagione: la stagione in cui è stato firmato
--  l'ultimo rinnovo. Nulla per chi non ha mai rinnovato in stagione (tutti,
--  al momento della migrazione: il canale esiste da stamattina).
-- ============================================================

alter table public.player_instances
  add column rinnovo_stagione smallint;

comment on column public.player_instances.rinnovo_stagione is
  'Stagione dell''ultimo rinnovo firmato in corso d''anno (design §10.4 bis). Si può ritrattare solo da quella successiva.';
