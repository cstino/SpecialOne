-- Test reversibile del pool: nessun giocatore viene eliminato. Il flag viene
-- consultato solo dalle funzioni che estraggono nuove carte o svincolati.
alter table public.players
  add column disponibile_estrazione boolean not null default true;

comment on column public.players.disponibile_estrazione is
  'Se false, il giocatore resta nel catalogo ma non entra in draft, aste o spin futuri.';

create index players_estrazione_attivi_idx
  on public.players (campionato)
  where disponibile_estrazione;

update public.players
set disponibile_estrazione = false
where overall < 66 and eta > 22;
...
begin
  ...
  with disponibili as (
    ...
    where p.disponibile_estrazione
      and p.campionato = any(v_lega.campionati_attivi)
...
as $$
  ...
  from public.players p
  where p.disponibile_estrazione
    and p.campionato = any(p_league.campionati_attivi)
...
begin
  ...
  from public.players p
  where p.disponibile_estrazione
    and p.campionato = any((v_ctx.v_league).campionati_attivi)
