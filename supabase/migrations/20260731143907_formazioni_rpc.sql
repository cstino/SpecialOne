-- ============================================================
--  FORMAZIONE: VALIDAZIONE E SALVATAGGIO SERVER-SIDE
-- ============================================================
...
begin
  ...
  if exists (
    ...
    where league_id = p_league_id and team_id = v_team.id
      and id = any(v_all) and infortunato_fino_a > 0
  ) then
    raise exception using errcode = '22023', message = 'La formazione contiene un giocatore indisponibile.';
  end if;
...
comment on function public.salva_formazione(bigint, smallint, text, bigint[], bigint[], bigint[]) is
  'Valida e salva una formazione slot-per-slot per la giornata indicata.';
