-- ============================================================
--  Chiude i due pezzi lasciati aperti dalla migrazione precedente
--  (20260901070000_vivaio_mercato_under.sql):
--   1. private.estrai_under() e private.risolvi_aste_under() esistono
--      gia' col guardiano orario Europe/Rome, ma nessun job pg_cron le
--      chiama ancora — stessa forma degli omologhi svincolati
--      (estrazione-svincolati alle *:02, risoluzione-aste alle *:03).
--   2. i prospetti vivaio non promossi restavano in vivaio_prospetti
--      per sempre. Ora finalizza_offseason li svincola e li rimette
--      in coda per il mercato UNDER, ma solo se hanno gia' passato
--      una stagione intera in cantiera (vedi commento sotto).
-- ============================================================

begin;

revoke all on function private.estrai_under() from public, anon, authenticated;
grant execute on function private.estrai_under() to service_role;

revoke all on function private.risolvi_aste_under() from public, anon, authenticated;
grant execute on function private.risolvi_aste_under() to service_role;
...
-- ------------------------------------------------------------
--  Svincolo automatico dei prospetti non promossi a fine off-season.
--
--  Un prospetto vinto all'asta durante la stagione X ha diritto a
--  giocarsi anche la stagione X+1 in cantiera prima di essere
--  rivalutato: si svincola solo chi e' gia' entrato in stagioni
--  precedenti a quella appena conclusa (entrata_stagione < stagione_da),
--  non chi ci e' arrivato durante la stagione appena chiusa. Stessa
--  meccanica del rilascio manuale (public.rilascia_vivaio): esce da
--  vivaio_prospetti e rientra in coda per la prossima estrazione UNDER.
-- ------------------------------------------------------------
...
begin
  ...
  -- Vivaio: chi non e' stato promosso ed e' gia' entrato in cantiera in
  -- una stagione precedente a quella appena conclusa torna sul mercato
  -- UNDER (in coda per la prossima estrazione), stessa strada del
  -- rilascio manuale.
  for v_vivaio in
    select id, league_id, player_id from public.vivaio_prospetti
    where league_id = p_league_id and entrata_stagione < v_off.stagione_da
  loop
    delete from public.vivaio_prospetti where id = v_vivaio.id;
    insert into private.rilasci_vivaio_in_coda (league_id, player_id)
    values (v_vivaio.league_id, v_vivaio.player_id)
...
grant execute on function private.finalizza_offseason(bigint) to service_role;
