-- ============================================================
--  ECONOMIA A TETTO SALARIALE — svincolo senza buonuscita, e i contratti
--  ereditati diventano al massimo annuali
--  docs/decisioni-economia.md §2, §4
--
--  Trovato ragionando su Real Fampionato: la buonuscita (design.md §9.5,
--  meta' dell'ingaggio per ogni stagione residua) e' ancora in vigore, e
--  BLOCCA lo svincolo se la squadra non ha abbastanza cassa per pagarla.
--  Regginho FC deve liberare 29,1 M€ di ingaggio per rientrare sotto il
--  tetto di 70M, ma i suoi contratti piu' pesanti (Grimaldo 11,3 M€ su 4
--  stagioni residue, Kobel 10,0 M€ su 4) costerebbero 22,6 e 20,0 M€ di
--  buonuscita — quasi tutto il suo budget cassa (47,4 M€), per liberare
--  ancora meno ingaggio del necessario. Le squadre col monte piu' alto,
--  quelle che piu' hanno bisogno di liberare spazio, sono spesso proprio
--  quelle senza cassa per farlo: e' la stessa identica trappola che ha
--  motivato tutta questa riprogettazione (§0 del documento).
--
--  Due correzioni, in quest'ordine:
--
--  1. Normalizzazione una tantum: nessun contratto attivo, in nessuna lega,
--     resta piu' lungo di "stagione corrente + 1". E' un dato coerente col
--     modello a contratti annuali (§2): da ora in avanti nessun rinnovo ne
--     crea uno piu' lungo, questo allinea quelli ereditati dal vecchio
--     sistema. Tocca 131 contratti in Real Fampionato, 193 in LegaBot.
--
--  2. svincola_giocatore_cassa_legacy non calcola piu' ne' blocca ne'
--     addebita la buonuscita. Il resto della funzione — sostituzione nelle
--     formazioni salvate, vincoli di rosa minima e portieri minimi — non e'
--     toccato. Il giocatore torna disponibile per il mercato degli
--     svincolati esattamente come gia' faceva (team_id = null, l'istanza
--     non si cancella): non serviva nessun cambiamento per quella parte.
-- ============================================================

update public.player_instances pi
set contratto_scadenza = sub.nuova_scadenza
from (
  select pi2.id, (l.stagione_corrente + 1)::smallint as nuova_scadenza
  from public.player_instances pi2
  join public.teams t on t.id = pi2.team_id
  join public.leagues l on l.id = t.league_id
  where pi2.contratto_scadenza > l.stagione_corrente + 1
    and not pi2.ritirato
) sub
where pi.id = sub.id;
