-- ============================================================
--  TITOLARI EFFETTIVI DELLA PARTITA
--
--  Bug segnalato dall'utente: nel dettaglio partita un giocatore con 90'
--  giocati compariva fra i "subentrati", e i titolari mostrati erano 10
--  invece di 11.
--
--  Causa: la Edge Function (buildLineup, supabase/functions/simula-giornata)
--  sostituisce un titolare infortunato al fischio d'inizio con il migliore
--  disponibile in panchina o in rosa -- ma quel rimpiazzo non veniva mai
--  scritto da nessuna parte, solo loggato. Da li' in poi il motore tratta il
--  rimpiazzante come un titolare qualsiasi, quindi puo' anche uscire durante
--  la partita per stanchezza: il suo minutaggio finale (parziale, non 90) lo
--  rende indistinguibile da un vero subentrato usando solo i minuti giocati.
--
--  `lineups.titolari` resta la scelta salvata PRIMA della partita (serve al
--  giocatore per rivedere cosa aveva schierato); qui si fotografa invece chi
--  e' sceso in campo DAVVERO, stesso pattern gia' in uso per modulo_home/
--  stile_home. Nessuna formula del motore toccata: solo dati aggiuntivi
--  esposti dalla Edge Function, che gia' li calcola in buildLineup().
-- ============================================================

alter table public.matches
  add column titolari_home bigint[] not null default '{}'::bigint[],
  add column titolari_away bigint[] not null default '{}'::bigint[];
...
create or replace function public.registra_risultato_partita(
  ...
  p_titolari_home bigint[] default '{}'::bigint[],
  p_titolari_away bigint[] default '{}'::bigint[]
)
returns jsonb
language plpgsql
volatile
...
comment on function public.registra_risultato_partita(
  bigint, bigint, text, text, text, text, smallint, smallint, jsonb, jsonb, jsonb, bigint[], bigint[]
) is 'Registra atomicamente una simulazione (modulo + stile + titolari effettivi) e aggiorna classifica e familiarita''; idempotente per fixture.';
