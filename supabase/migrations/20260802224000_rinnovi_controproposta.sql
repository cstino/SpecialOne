-- Rinnovi: prima del rifiuto definitivo il giocatore fa una sola
-- controproposta, rendendo esplicita la richiesta reale.

alter table public.contract_renewals
  drop constraint if exists contract_renewals_stato_check;

alter table public.contract_renewals
  add constraint contract_renewals_stato_check
  check (stato in ('in_attesa','controproposta','accettato','rifiutato','scaduto','ritirato'));
