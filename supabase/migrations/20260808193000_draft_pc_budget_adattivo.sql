...
-- Il PC non deve consumare il budget nei primi pick confidando che esistano
-- abbastanza carte da 0,5 M. Il bersaglio economico viene ricalcolato a ogni
-- scelta sul budget draft ancora disponibile e sugli slot ancora vuoti.
...
begin
  ...
  loop
    ...
    if v_league.modalita_draft = 'by_role' then
      -- Liberta' effettiva: il PC non segue una scaletta fissa, pur potendo
      -- correggere il reparto tramite il fallback se il pool e' scarso.
    ...
    -- Oscillazione controllata: simula un roll e produce rose differenti,
    -- ma mantiene la spesa attorno alla media necessaria per arrivare a 24.
    ...
    if not found then
      -- Reroll automatico su qualsiasi reparto, sempre con gli stessi limiti.
      select p.* into v_player
      from public.players p
      where p.disponibile_estrazione
        and (p.elite_globale or p.campionato = any(v_league.campionati_attivi))
